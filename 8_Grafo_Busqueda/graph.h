...
void Graph::loadGraphList(string name, int a){
	...
	while (pos < name.length()) {
		u = stoi(name.substr(pos+1,pos+1));
		v = stoi(name.substr(pos+3,pos+3));
		addEdgeAdjList(u,v);
		pos += 6; 
	}
...
string Graph::DFS(int start, int goal){
	...
	string r = depthHelper(start, goal, st, visited, paths);

	return r + print_path(paths, start, goal);
}
...
string Graph::BFS(int start, int goal){
	...
	string r = breadthHelper(start, goal, qu, visited, paths);
